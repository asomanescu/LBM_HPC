...
// #pragma omp parallel for shared(tmp_cells) private(ii, kk, ri, rj, u_sq, local_density, t, u, d_equ) schedule(dynamic, 4)
    tot_u = 0.0;
    for (ii = 0; ii< params.nx * params.ny; ii++) {
        ri = ii/params.nx;
        rj = ii%params.nx;
        if (ri == 0) {   
         y_s = ri + params.ny - 1;    
        ...
        t[1] = cells[ri*params.nx + x_w].speeds[1];
        t[2] = cells[y_s*params.nx + rj].speeds[2];
        t[3] = cells[ri*params.nx + x_e].speeds[3];
        t[4] = cells[y_n*params.nx + rj].speeds[4];
        ...
        if (obstacles[ii]) {
            ...
            /* velocity squared */
            const float u_sq = u[1] * u[1] + u[2] * u[2];
